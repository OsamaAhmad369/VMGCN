...
class ASTGCN_block(nn.Module):
    def __init__(self, device, in_channels, order, nb_chev_filter, nb_time_filter, time_strides, cheb_polynomials, node_num, seq_len):
        ...
        self.CAt = Channel_Attention_layer(device, in_channels-2, node_num, seq_len)
...
class Channel_Attention_layer(nn.Module):
    def __init__(self, device, in_channels, node_num, seq_len):
        ...
        self.V1 = nn.Parameter(torch.FloatTensor(seq_len).to(device))
        self.V2 = nn.Parameter(torch.FloatTensor(node_num, seq_len).to(device))
        self.V3 = nn.Parameter(torch.FloatTensor(node_num).to(device))
        self.be = nn.Parameter(torch.FloatTensor(1, in_channels, in_channels).to(device))
        self.Ve = nn.Parameter(torch.FloatTensor(in_channels, in_channels).to(device))
        self.thres=nn.Parameter(torch.FloatTensor(1).to(device))
